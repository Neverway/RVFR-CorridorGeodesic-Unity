...
namespace Neverway.Framework.Cartographer
{
    public class WB_LevelEditor_Inspector : MonoBehaviour
    {
        ...
        //=-----------------=
        [SerializeField]
        private GameObject stringField, intField, floatField, boolField, spriteStringField, fieldListRoot;
            ...
            for (int i = 0; i < fields.Count; i++)
            {
                switch (target.storedVariableData[i].type)
                {
                    case "System.String":
                        ...
                        {
                            target.SetVariableValue(null, target.storedVariableData[i].name,
                                fields[i].transform.GetChild(1).GetChild(0).GetComponent<TMP_Text>().text);
                        }
                        else
                        {
                            target.SetVariableValue(null, target.storedVariableData[i].name,
                                fields[i].transform.GetChild(1).GetComponent<TMP_InputField>().text);
                        }
                    ...
                    case "System.Boolean":
                        target.SetVariableValue(null, target.storedVariableData[i].name,
                            fields[i].transform.GetChild(1).GetComponent<Toggle>().isOn.ToString());
                        break;
                }
            }
        }
                ...
                switch (target.storedVariableData[i].type)
                {
                    case "System.String":
                        if (target.storedVariableData[i].name.StartsWith("sprite"))
                        {
                            ...
                            field.transform.GetChild(1).GetChild(0).GetComponent<TMP_Text>().text =
                                target.storedVariableData[i].value;
                        }
                        else
                        {
                            field = Instantiate(stringField, fieldListRoot.transform);
                            field.transform.GetChild(1).GetComponent<TMP_InputField>().text =
                                target.storedVariableData[i].value;
                        }
                    ...
                    case "System.Boolean":
                        ...
                        field.transform.GetChild(1).GetComponent<Toggle>().isOn =
                            target.storedVariableData[i].value == "True";
                        break;
                }
