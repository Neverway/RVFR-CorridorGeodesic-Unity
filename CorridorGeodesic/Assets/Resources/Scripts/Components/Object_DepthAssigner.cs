    ...
    public class Object_DepthAssigner : MonoBehaviour
    {
        ...
        private void AssignColliders()
        {
            switch (depthLayer)
            {
                case 0:
                {
                    foreach (var collider in targetedColliders)
                    {
        ...
        private void AssignSpriteRenderers()
        {
            switch (depthLayer)
            {
                case 0:
                {
                    foreach (var spriteRender in targetedSpriteRenderers)
                    {
                        spriteRender.sortingLayerName = "Depth Layer 1";
                    }
                ...
                case 1:
                {
                    foreach (var spriteRender in targetedSpriteRenderers)
                    {
                        spriteRender.sortingLayerName = "Depth Layer 2";
                    }
                ...
                case 2:
                {
                    foreach (var spriteRender in targetedSpriteRenderers)
                    {
                        spriteRender.sortingLayerName = "Depth Layer 3";
                    }
        ...
        private IEnumerator FallForSeconds()
        {
            ...
            {
                gameObject.GetComponent<Pawn>().isPaused = true;
            }
            ...
            if (gameObject.GetComponent<Pawn>())
            {
                gameObject.GetComponent<Pawn>().isPaused = false;
            }
