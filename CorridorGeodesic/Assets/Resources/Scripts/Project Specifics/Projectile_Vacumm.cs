...
public class Projectile_Vacumm : Projectile_VacummNew
{
    ...
    // Reference Variables
    //=-----------------=
    [FormerlySerializedAs("geoFolder")] [HideInInspector] public Item_Geodesic_Utility_NixieCross nixieCross; // Get a reference to the gun that spawned the projectile, so we know who to give ammo to on a lifetime expiration
    ...
    public void KillProjectile (bool removeProjectileFromList = true)
    {
        ...
        if (nixieCross)
        {
            if (nixieCross.currentAmmo < nixieCross.maxAmmo)
            {
                nixieCross.currentAmmo++;
            }

            if (removeProjectileFromList) nixieCross.deployedInfinityMarkers.Remove(gameObject);
        }
