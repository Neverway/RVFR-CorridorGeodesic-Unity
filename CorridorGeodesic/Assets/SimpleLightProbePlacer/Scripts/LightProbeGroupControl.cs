...
namespace SimpleLightProbePlacer
{
    ...
    public class LightProbeGroupControl : MonoBehaviour
    {
        ...
        {
            LightProbeGroup.probePositions = new Vector3[0]; // Clear the positions
            m_mergedProbes = 0;
        }
        ...
        {
            m_lightProbeGroup.probePositions = positions.Select(x => transform.InverseTransformPoint(x)).ToArray(); // Use m_lightProbeGroup
        }
